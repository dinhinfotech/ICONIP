...
\begin{abstract}
The amount of relational data is increasing rapidly. These datasets are best represented as networks or graphs. A key characteristic of this type of data collections is the sparseness and dynamic nature. A crucial computational task is then the ''link prediction problem'' which allows to suggest possible collaborators for scientists in social networks, or to discover unknown interactions between proteins to explain the mechanism of a disease in biological networks, or to suggest novel products to be bought to a customer in a e-commerce recommendation system. Most approaches however do not effectively exploit the topological contextual information available in the neighborhood of each edge. Here we cast the problem as a binary classification task over specific subgraphs and employ efficient graph kernels to solve it and show empirically state-of-the-art results on several benchmark datasets. 
