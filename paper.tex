...
\textit{neighborhood} with radius $r$ of $u$ and the relative edges with endpoints in $N_r(u)$. The degree of a node $u$ denoted as $d(u)$ is the cardinality of its neighborhood $d(u) = |\mathcal{N}_{1}^{u}|$. We also notate the maximum node degree in the graph $G$ as $d(G)$, $d(G) = max\lbrace d(u)\ |\ u \in V \rbrace$
